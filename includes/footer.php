...
if (empty($footer_link_data['data']) && function_exists('cmsLoadFooterBootstrap')) {
    $footerBootstrap = cmsLoadFooterBootstrap();
...
// ---------------------------
// Header & Footer meta data
// ---------------------------
$headerPart = cmsLayoutPart($header_footer_data, 'header-aps-agra')
    ?? cmsLayoutPartByType($header_footer_data, 'header')
    ?? ($header_footer_data[0] ?? []);
$headerMeta = is_array($headerPart['meta_data'] ?? null) ? $headerPart['meta_data'] : cmsLayoutMeta($header_footer_data, 'header-aps-agra');

$footerPart = cmsLayoutPart($header_footer_data, 'footer-aps-agra')
    ?? cmsLayoutPartByType($header_footer_data, 'footer')
    ?? ($header_footer_data[1] ?? []);
$footerMeta = is_array($footerPart['meta_data'] ?? null) ? $footerPart['meta_data'] : cmsLayoutMeta($header_footer_data, 'footer-aps-agra');
...
// ---------------------------
// Extract Important Links & Quick Links
// ---------------------------
$impLinks = [];
$quickLinks = [];

// First, try the direct path (single group API)
if (!empty($footer_link_data['data']['links']) && is_array($footer_link_data['data']['links'])) {
    $impLinks = $footer_link_data['data']['links'];
} else {
    // Multiple groups or alternative structures
    $possibleGroups = [];
    if (!empty($footer_link_data['data']) && is_array($footer_link_data['data'])) {
        $possibleGroups = $footer_link_data['data'];
        if (isset($possibleGroups['important-links']) || isset($possibleGroups['quick-links'])) {
            $temp = [];
            foreach ($possibleGroups as $slug => $group) {
                if (is_array($group) && isset($group['links'])) {
                    $temp[] = $group;
                }
            }
            if (!empty($temp)) $possibleGroups = $temp;
        }
    } elseif (!empty($footer_link_data) && is_array($footer_link_data)) {
        $possibleGroups = $footer_link_data;
    }

    foreach ($possibleGroups as $group) {
        if (!is_array($group)) continue;
        $slug = $group['slug'] ?? $group['name'] ?? '';
        $links = $group['links'] ?? $group['menu_items'] ?? [];
        if (empty($links)) continue;
        if (stripos($slug, 'important') !== false || stripos($slug, 'imp') !== false) {
            $impLinks = $links;
        } elseif (stripos($slug, 'quick') !== false) {
            $quickLinks = $links;
        }
    }
}
...
if (empty($impLinks) && !empty($footer_link_data['important-links']['links'])) {
    $impLinks = $footer_link_data['important-links']['links'];
}
if (empty($quickLinks) && !empty($footer_link_data['quick-links']['links'])) {
    $quickLinks = $footer_link_data['quick-links']['links'];
}

// ---------------------------
// Build tree structure from flat links array (for nested dropdowns)
// ---------------------------
function buildLinkTree($links, $parentId = null) {
    $branch = [];
    foreach ($links as $link) {
        $linkParentId = $link['parent_id'] ?? null;
        if ($linkParentId == $parentId) {
            $children = buildLinkTree($links, $link['id']);
            if ($children) {
...
// ---------------------------
// Recursive render function for nested links
// ---------------------------
if (!function_exists('renderNestedFooterLinks')) {
    function renderNestedFooterLinks($links, $linkClass, $isSub = false) {
        if (empty($links)) {
            return;
        }

        $subLinkClass = 'block transition py-2 px-2 text-blue-main hover:text-red-500';
        echo $isSub ? '<ul class="text-sm">' : '<ul class="grid grid-cols-1 gap-2 mt-2">';

        foreach ($links as $link) {
            $rawUrl = !empty($link['url']) ? $link['url'] : (!empty($link['link']) ? $link['link'] : '#');
            $url = cmsMenuUrl($rawUrl);
            $title = !empty($link['title']) ? $link['title'] : (!empty($link['name']) ? $link['name'] : 'Link');
            $target = !empty($link['target']) ? $link['target'] : '_self';
            $hasChildren = !empty($link['children']);

            if ($isSub) {
                echo '<li class="border-b border-b-[#e5e7eb] last:border-b-0 hover:border-l-[#002A5B] hover:border-l-2">';
                echo '<a href="' . htmlspecialchars($url, ENT_QUOTES, 'UTF-8') . '" target="' . htmlspecialchars($target, ENT_QUOTES, 'UTF-8') . '" class="' . $subLinkClass . '">' . htmlspecialchars($title, ENT_QUOTES, 'UTF-8') . '</a>';
                echo '</li>';
                continue;
            }
            ...
            echo '<li>';
            if ($hasChildren) {
                echo '<button type="button" class="toggle-submenu ' . $linkClass . ' transition text-left inline-flex items-center relative my-1 gap-1 group" aria-expanded="false" aria-label="Toggle submenu">';
                echo htmlspecialchars($title, ENT_QUOTES, 'UTF-8');
                echo '<span class="footer-submenu-caret text-[10px] leading-none" aria-hidden="true">▼</span>';
                echo '</button>';
                echo '<div class="submenu hidden z-10 mt-1 bg-white w-44 rounded shadow-md">';
                renderNestedFooterLinks($link['children'], $linkClass, true);
                echo '</div>';
            } else {
                echo '<a href="' . htmlspecialchars($url, ENT_QUOTES, 'UTF-8') . '" target="' . htmlspecialchars($target, ENT_QUOTES, 'UTF-8') . '" class="' . $linkClass . '">' . htmlspecialchars($title, ENT_QUOTES, 'UTF-8') . '</a>';
            }
            echo '</li>';
        }
        echo '</ul>';
...
// Build tree for important links
$impLinksTree = buildLinkTree($impLinks);
...
<footer class="py-5 px-4 pmt-20 bg-blue-main flex flex-col md:flex-row justify-around">
    <!-- Left Column: Logo & Contact -->
    <div class="md:mt-3 md:order-1 order-1 mt-8">
        <div>
            <a href="<?= site_base_url() ?>">
                <img src="<?= htmlspecialchars($footerLogo) ?>" alt="<?= cms_image_alt($footerMeta, 'Allenhouse Agra') ?>"
                    class="w-[236px] md:w-[200px]">
            </a>
        </div>
        <div class="mt-5">
            <div class="mt-4 md:mt-6">
                <div class="flex gap-2 mb-3">
                    <p class="text-white font-[400] 2xl:text-[17px] xl:text-[14px] lg:text-[13px] md:text-[12px] text-[12px]">
                        <?= nl2br(htmlspecialchars($footerAbout !== '' ? $footerAbout : 'Plot No. C2, Sector-C2, Shastripuram, Agra' . "\n" . 'Uttar Pradesh - 282007')) ?>
                    </p>
                </div>
                <div class="flex gap-2 mb-3 items-center">
                    ...
                    <p class="text-white font-[400] 2xl:text-[17px] xl:text-[14px] lg:text-[13px] md:text-[12px] text-[12px]"><a href="tel:<?= htmlspecialchars(preg_replace('/\s+/', '', $footerPhone)) ?>"><?= htmlspecialchars($footerPhone) ?></a></p>
                </div>
                <div class="flex gap-2 mb-3 items-center">
                    ...
                    <a class="text-white font-[400] 2xl:text-[17px] xl:text-[14px] lg:text-[13px] md:text-[12px] text-[12px]" href="mailto:<?= htmlspecialchars($footerEmail) ?>"><?= htmlspecialchars($footerEmail) ?></a>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="flex justify-between md:w-[30%] md:order-2 order-2">
        <div class="mt-3">
            ...
            <?php if (!empty($impLinksTree)): ?>
                <?php renderNestedFooterLinks($impLinksTree, $linkClass); ?>
            <?php else: ?>
        ...
        <div class="mt-2 md:mt-4">
            <div>
                ...
                <?php if (!empty($quickLinks)): ?>
                    <?php 
                    // Quick links are flat (no hierarchy) so we render directly
                    ...
                    foreach ($quickLinks as $link) {
                        ...
                        $title = !empty($link['title']) ? $link['title'] : (!empty($link['name']) ? $link['name'] : 'Link');
                        $target = !empty($link['target']) ? $link['target'] : '_self';
                        echo '<li><a href="' . htmlspecialchars($url, ENT_QUOTES, 'UTF-8') . '" target="' . htmlspecialchars($target, ENT_QUOTES, 'UTF-8') . '" class="' . $linkClass . '">' . htmlspecialchars($title, ENT_QUOTES, 'UTF-8') . '</a></li>';
                    }
                    echo '</ul>';
                    ?>
                <?php endif; ?>
    ...
    <!-- Right Column: Social, App, Newsletter -->
    <div class="mt-5 order-3 md:w-[25%] md:mt-8">
        <!-- Social Media -->
        <div>
            <h2 class="md:text-[22px] text-[18px] font-[600]" style="color:#B4D7FF">Social Media</h2>
            <ul class="flex md:gap-4 gap-2 mt-2 md:mt-4">
                <?php if (!empty($footerMeta['footer_youtube'])): ?>
                <li><a href="<?= htmlspecialchars($footerMeta['footer_youtube']) ?>" target="_blank" rel="noopener" class="real-should-display transition-all">
                    <svg class="not-real-hide sm:w-10 sm:h-10 w-8 h-8" viewBox="0 0 24 24" fill="none"><path fill="#fff" d="m10 15l5.19-3L10 9zm11.56-7.83c.13.47.22 1.1.28 1.9c.07.8.1 1.49.1 2.09L22 12c0 2.19-.16 3.8-.44 4.83c-.25.9-.83 1.48-1.73 1.73c-.47.13-1.33.22-2.65.28c-1.3.07-2.49.1-3.59.1L12 19c-4.19 0-6.8-.16-7.83-.44c-.9-.25-1.48-.83-1.73-1.73c-.13-.47-.22-1.1-.28-1.9c-.07-.8-.1-1.49-.1-2.09L2 12c0-2.19.16-3.8.44-4.83c.25-.9.83-1.48 1.73-1.73c.47-.13 1.33-.22 2.65-.28c1.3-.07 2.49-.1 3.59-.1L12 5c4.19 0 6.8.16 7.83.44c.9.25 1.48.83 1.73 1.73"/></svg>
                ...
                <?php endif; ?>
                <?php if (!empty($footerMeta['footer_facebook'])): ?>
                <li><a href="<?= htmlspecialchars($footerMeta['footer_facebook']) ?>" target="_blank" rel="noopener" class="real-should-display transition-all">
                ...
                <?php endif; ?>
                <?php if (!empty($footerMeta['footer_instagram'])): ?>
                <li><a href="<?= htmlspecialchars($footerMeta['footer_instagram']) ?>" target="_blank" rel="noopener" class="real-should-display transition-all">
                ...
                <?php endif; ?>
                <?php if (!empty($footerMeta['footer_linkedin'])): ?>
                <li><a href="<?= htmlspecialchars($footerMeta['footer_linkedin']) ?>" target="_blank" rel="noopener" class="real-should-display transition-all">
        ...
        <!-- Allen Care App -->
        <div>
            <h2 class="md:text-[22px] text-[18px] font-[600] mt-3 md:mt-5" style="color:#B4D7FF">Allen Care App</h2>
            <div class="flex items-center mt-3 gap-1">
                ...
                <?php endif; ?>
                <?php if ($iosAppLink !== ''): ?>
                <a href="<?= htmlspecialchars($iosAppLink) ?>" target="_blank" rel="noopener"><img class="w-[140px]" src="<?= htmlspecialchars($iosBadge) ?>" alt="Download iOS app"></a>
                <?php endif; ?>
            </div>
        </div>
        <!-- Newsletter -->
...
<style>.whatsapp-button { position: fixed; right: 15px; bottom: 15px; z-index: 999; } .whatsapp-button img { width: 60px; height: 60px; border-radius: 50%; }</style>
