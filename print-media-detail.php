...
$data = $print_media_data;

if (!$data || $data['status'] !== 'success') {
    echo "Failed to fetch or decode API data.";
    exit;
}
...
// Extract Title and Description from nested printmediadetails
$galleryTitle = 'Print Media';
$galleryDescription = '';

if (!empty($gallery['printmediadetails'][0])) {
    $detail = $gallery['printmediadetails'][0];
    $galleryTitle = $detail['title'] ?? $galleryTitle;
    $galleryDescription = $detail['description'] ?? '';
}
...
<head>
    ...
    <title><?= htmlspecialchars($galleryTitle) ?> | AllenHouse Agra</title>
