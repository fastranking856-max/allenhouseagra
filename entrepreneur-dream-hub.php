...
// Get raw CMS data for Entrepreneur's Dream Hub
$raw_page = $GLOBALS['api_cache']['entrepreneur_dream_hub_data'] ?? [];
...
if ($hero_banner && !empty($hero_banner['content_heading'])) {
    ...
    $banner_html = '
    <div>
        <h2 class="text-[32px] sm:hidden block font-[700] text-white text-left mb-5 sm:mb-8 hr-line relative leading-9 pl-4">Entrepreneur\'s Dream Hub</h2>
    </div>
    <div class="md:w-[100%]">
        <h2 class="sm:text-[32px] sm:block hidden font-[700] text-white text-left sm:mb-1 hr-line relative leading-9 ml-[7rem]">Entrepreneur\'s Dream Hub</h2>
    </div>';
}
?>
...
<head>
    ...
    <link rel="canonical" href="https://allenhouseagra.com/entrepreneur-dream-hub" />
    <title>Entrepreneurship Skills for Students | Allenhouse Agra</title>
    <meta name="description" content="APS Agra equips students with entrepreneurship skills to lead, innovate, and thrive in a real-world business environment.">
...
<div class="main relative mb-[40px] sm:mb-[120px]">
    ...
    <div class="flex m-5" aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse whitespace-nowrap overflow-x-auto">
            ...
            <li><div class="flex items-center"><svg class="rtl:rotate-180 w-3 h-3 text-blue-main mx-1" ...><path d="m1 9 4-4-4-4"/></svg><a href="entrepreneur-dream-hub" class="ms-1 sm:text-sm text-xs font-medium text-blue-main">Entrepreneur's Dream Hub</a></div></li>
        </ol>
    </div>
    ...
    <div class="mt-8 mx-3 2xl:w-[1280px] lg:w-[1024px] md:w-[767px] sm:w-[640px] sm:mx-auto sm:px-5 px-3">
        <?php if (empty($sections)): ?>
            <p class="font-[700] text-center text-red-600">Entrepreneur's Dream Hub content will be added soon.</p>
        <?php else: ?>
            <?php foreach ($content_blocks as $block): ?>
                <div class="mt-10 relative">
                    ...
                    if (!empty($block['columns']) && is_array($block['columns']) && count($block['columns']) >= 2) {
                        ...
                        if (isset($col1['content_type']) && $col1['content_type'] === 'image') {
                            ...
                            <div class="sm:w-[40%] mb-6 sm:mb-0">
                                <img src="<?= htmlspecialchars($img_url) ?>" alt="Entrepreneur's Dream Hub" class="w-[100%] rounded-lg shadow-lg">
                            </div>
                            <?php
                        }
                        // Text column
                    ...
                    // Fallback: plain content block (full description, optionally split)
