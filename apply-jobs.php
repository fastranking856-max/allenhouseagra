...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_FILES['resume']) || $_FILES['resume']['error'] !== UPLOAD_ERR_OK) {
        ...
        if ($resume['size'] > 2 * 1024 * 1024) $errors[] = "Resume must be â‰¤ 2MB.";
    ...
    if (isset($_FILES['cover_letter']) && $_FILES['cover_letter']['error'] === UPLOAD_ERR_OK) {
        ...
        if ($cover['size'] > 2 * 1024 * 1024) $errors[] = "Cover letter must be â‰¤ 2MB.";
        $finfo = finfo_open(FILEINFO_MIME_TYPE);
            ...
            <div class="max-w-5xl mx-auto p-6 space-y-8 bg-white shadow-xl mt-10 rounded-xl">
                <form id="applicationForm" method="post" action="" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div>
                        ...
                        <p id="resumeError" class="text-sm text-red-500 hidden mt-1">Only PDF files â‰¤ 2MB are allowed.</p>
                    </div>
                    ...
                    <div>
                        ...
                        <p id="coverError" class="text-sm text-red-500 hidden mt-1">Only PDF files â‰¤ 2MB are allowed.</p>
                    </div>
