...
$jobid = $_GET['id'] ?? null;
$j_data = cmsFindJobInResponse($jobs_data, $jobid);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    curl_setopt_array($ch, [
        CURLOPT_URL => rtrim(API_BASE_URL, '/') . '/apply-job/' . SCHOOL_ID,
        ...
        CURLOPT_SSL_VERIFYPEER => false, // Use only for testing
            ...
            <div class="max-w-5xl mx-auto p-6 space-y-8 bg-white shadow-xl mt-10 rounded-xl">
                ...
                <div class="space-y-2">
                    ...
                    <p class="text-sm text-gray-600 flex items-center gap-2">
                        ...
                        Plot No. C 2, Sector - C 2, Shastripuram Agra, 282007
                    </p>
                </div>
                     ...
                     <script>
                        document.getElementById("viewDesc").addEventListener("click", function() {
                            ...
                            if (/Mobi|Android|iPhone|iPad/i.test(navigator.userAgent)) {
                                // On mobile â†’ open in default viewer (new tab)
                                window.open(pdfUrl, "_blank");
                            } else {
                                // On desktop â†’ toggle inline PDF
                ...
                <!-- Application Form -->
                   <form id="applicationForm" method="post" action="" class="space-y-6" enctype="multipart/form-data" novalidate>
                    ...
                    <div>
                        ...
                        <p id="resumeError" class="text-sm text-red-500 hidden mt-1">File must be â‰¤ 2MB.</p>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="file" name="cover_latter" id="cover_latter" accept=".pdf,.doc,.docx,image/*"
                            class="w-full p-2 border border-dashed border-gray-400 rounded-md" />
                        <p id="coverError" class="text-sm text-red-500 hidden mt-1">File must be â‰¤ 2MB.</p>
                    </div>
